  ...
  const [signupAlert,setSignUpAlert] = useState(false);
    ...
    useEffect(() => {
      const callFetchFunc = async ()=> {
        ...
        const prodList:Product[] = (await fetchProductList(`http://localhost:3000/api/wearista?limit=${limit}&page=${page}`)).map((e) => ({...e, productQuantity:1}));
        dispatch({type:LOADPRODUCT,payload:prodList});
        console.log(prodList)
        const backUp = (await fetchProductList(`http://localhost:3000/api/wearista`)).map((e) => ({...e, productQuantity:1}));
        console.log(backUp);
