  ...
  // const [signupAlert,setSignUpAlert] = useState(false);
    ...
    useEffect(() => {
      const callFetchFunc = async ()=> {
        ...
        const prodList:Product[] = (await fetchProductList(`${process.env.NEXT_PUBLIC_WEARISTA}/api/wearista`)).map((e) => ({...e, productQuantity:1}));
        dispatch({type:LOADPRODUCT,payload:prodList});
        console.log(prodList)
        const backUp = (await fetchProductList(`${process.env.NEXT_PUBLIC_WEARISTA}/api/wearista`)).map((e) => ({...e, productQuantity:1}));
        console.log(backUp);
...
useEffect(() => {
  ...
  //HANDLE WISH LIST
  useEffect(() => {
    Cookies.set('wishlist',JSON.stringify(cartData.wishList), { expires: 7 })
  },[cartData.wishList]);
